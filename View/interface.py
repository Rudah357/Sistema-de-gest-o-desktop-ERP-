...
from View.tela_listar import listar_produtos
from View.tela_atualizar import atualizar_produtos
from View.tela_grafico import grafico
from View.tela_deletar import deletar_produto
from View.tela_config import config
...
def app():
    ...
    frame_menu_principal = ctk.CTkFrame(interface, fg_color="#3083F0", bg_color="#3083F0", height=80)
    frame_menu_principal.pack(side="top", fill="x")

    label_menu_principal = ctk.CTkLabel(frame_menu_principal, text="Sistema de Gestão V-3", font=("Segoe UI", 28), text_color="white")
    label_menu_principal.pack(pady=25, padx=10)

    borda_frame = ctk.CTkFrame(interface, fg_color="#2D79DD", bg_color="#3083F0", height=20)
